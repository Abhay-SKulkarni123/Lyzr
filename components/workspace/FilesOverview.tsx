...
const entries = [
  ...
  { path: "components/dashboard/RevenueChart.tsx", description: "Monthly revenue chart", type: "tsx", size: "1.9 KB" },
    ...
    <section className="mx-auto w-full max-w-[760px] rounded-lg border border-white/[0.08] bg-[#10141d] p-4 sm:p-6" aria-label="Project files overview">
      <div className="mb-5 flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {updated > 0 && (
            <span className="flex items-center gap-1.5 rounded-full border border-emerald-400/25 bg-emerald-400/10 px-2.5 py-1 text-[9px] text-emerald-300"><Layers3 className="h-3 w-3" /> {updated} updated</span>
          )}
        ...
        {entries.map((entry) => {
          ...
          return (
            <button key={entry.path} className="flex w-full items-center gap-3 border-b border-white/[0.05] px-3 py-3 text-left last:border-0 hover:bg-white/[0.035]" onClick={() => onOpenFile(entry.path)} type="button">
              ...
              {isModified && (
                <span className="flex shrink-0 items-center gap-1 rounded border border-emerald-400/25 bg-emerald-400/10 px-1.5 py-0.5 text-[8px] text-emerald-300"><span className="h-1 w-1 rounded-full bg-emerald-400" />Updated</span>
              )}
