...
type CodeSurfaceProps = { selectedFile: string };

export function CodeSurface({ selectedFile }: CodeSurfaceProps) {
  ...
  return (
    <section className="flex h-full min-h-[420px] flex-col overflow-hidden rounded-lg border border-white/[0.08] bg-[#0c1018]" aria-label="Code preview">
      ...
      <div className="workspace-scrollbar flex-1 overflow-auto py-3 font-mono text-[11px] leading-[22px]">
        {lines.map((line, index) => (
          <div key={`${index}-${line}`} className="flex min-w-max px-4 hover:bg-white/[0.025]">
            <span className="mr-5 inline-block w-5 select-none text-right text-slate-700">{index + 1}</span>
